...
//cv::imshow("cur_image", cur_kf->image);
//cv::waitKey(2);
    ...
    while(true)
    {
        ...
        if (cur_index != -1)
        {
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                for (int j = 1; j < 5; j++)
                {
                    /// {  sequence_array[i] == sequence_array[i-j]  }这个判断可能是为了确保是在同一个关键帧id处往回lookback，可能是因为迭代器iter是无序的关系吧，不是顺序的（顺序要多做一次sort，太慢了？）
                  if (i - j >= 0 && sequence_array[i] == sequence_array[i-j])
                  {
                      /// 这跟我的frames_to_lookback是一样的，这里往回看5帧，理论上这里的connection关系应该用nonlinear factor recovery恢复出来，这里直接取了world系下的vio_pose，也能用，但感觉不精致

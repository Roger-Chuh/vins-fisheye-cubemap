...
static void ShowDetails(KeyFrame* old_kf, KeyFrame* new_kf, vector<cv::Point2f> point_2d_uv_old, vector<cv::Point2f> point_2d_uv_cur){

    cv::Mat gray_img, loop_match_img;
    cv::Mat old_img = old_kf->image;
    cv::hconcat(new_kf->image, old_img, gray_img);
    cvtColor(gray_img, loop_match_img, CV_GRAY2RGB);
    for(int i = 0; i< (int)new_kf->keypoints.size(); i++)
    {
        cv::Point2f cur_pt = new_kf->keypoints[i].pt;
        cv::circle(loop_match_img, cur_pt, 5, cv::Scalar(0, 255, 0));
    }
    for(int i = 0; i< (int)point_2d_uv_cur.size(); i++)
    {
        cv::Point2f cur_pt = point_2d_uv_cur[i];
    ...
    for(int i = 0; i< (int)point_2d_uv_old.size(); i++)
    {
        cv::Point2f old_pt = point_2d_uv_old[i];
        old_pt.x += COL;
        cv::circle(loop_match_img, old_pt, 5, cv::Scalar(255, 0, 0));
    }

    ostringstream path;
    // path << "/home/tony-ws1/raw_data/loop_image/"
    path << "/home/roger/vins-fisheye2/src/VINS-Fisheye/debug/draw/"
         << new_kf->index << "-"
...
void KeyFrame::computeBRIEFPoint()
{
    ...
    /// 在原图提取新的特征点并计算描述符
    printf("enter computeBRIEFPoint...\n");
	BriefExtractor extractor(BRIEF_PATTERN_FILE.c_str());
                                      ...
                                      vector<uchar> &status)
{
    ...
    printf("enter FundmantalMatrixRANSAC...\n");
                         ...
                         Eigen::Vector3d &PnP_T_old, Eigen::Matrix3d &PnP_R_old)
{
    ...
    double inlier_pix_thr = 20; // 40; //         100.0;     // 40.0;  // 20.0; // 15.0;  // 10.0;


    if (CV_MAJOR_VERSION < 3)
        solvePnPRansac(matched_3d, matched_2d_old_norm, K, D, rvec, t, true, 100, inlier_pix_thr / 460.0, 100, inliers);
    else
    {
        if (CV_MINOR_VERSION < 2)
            solvePnPRansac(matched_3d, matched_2d_old_norm, K, D, rvec, t, true, 100, sqrt(inlier_pix_thr / 460.0), 0.99, inliers);
        else

            // solvePnPRansac(matched_3d, matched_2d_old_norm, K, D, rvec, t, true, 100, inlier_pix_thr / 460.0, 0.99, inliers);


            solvePnPRansac(matched_3d, matched_2d_old_norm, K, D, rvec, t, true, 100, inlier_pix_thr / 460.0, 0.95, inliers);
    ...
    std::cout<<"inliers ratio: ["<<inliers.rows<<" / "<<matched_2d_old_norm.size()<<"]"<<std::endl;
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
    ...
    //ShowDetails(old_kf, this, matched_2d_old, matched_2d_cur);
	...
	//printf("search by des finish\n");
		...
		if (DEBUG_IMAGE)
	    {
	        ...
	        //path <<  "/home/tony-ws1/raw_data/loop_image/"
	        path << "/home/roger/vins-fisheye2/src/VINS-Fisheye/debug/"
	                << index << "-"
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    reduceVector(matched_id, status);
	    	...
	    	//if (true) {
                ...
                //std::cout<<"this->image size: "<<this->image.size<<std::endl;
                //cv::imshow("this->image", this->image);
                //cv::waitKey(2);
...
//cv::imshow("loops",loop_match_img);
//cv::waitKey(2);

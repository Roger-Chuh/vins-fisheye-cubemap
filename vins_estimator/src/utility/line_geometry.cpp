...
Vector4d plk_to_orth(Vector6d plk)
{
    ...
    /// w0 = [norm(n0) norm(v0)]
    /// w  = w0./norm(w0)
    ///    = [norm(n0)/sqrt(n0.^2 + v0.^2) norm(v0)/sqrt(n0.^2 + v0.^2)]
    ///    = [n   v]
    ///    = [w1 w2]


    /// roger note
    /// 直线方向归一化，法向量不归一化，这里归一化了，说明w表示plk坐标中的方向向量部分
    // todo::
    /// [ cos(phi)  -sin(phi)
    ///   sin(phi)   cos(phi)]
    /// [ w1  -w2
    ///   w2   w1 ]
    /// dist = cos(phi) / sin(phi) = w1 / w2

    // todo 这一步归一化很重要，消除了尺度因子，这就是好的参数化方式带来的便利，隐式约束
    w = w/w.norm();
...
Vector6d orth_to_plk(Vector4d orth)
{
    ...
    Matrix3d R;
    // todo::
    /// this is NOT rodrigues, this is eular angle
    ...
    R <<
      c2 * c3,   s1 * s2 * c3 - c1 * s3,   c1 * s2 * c3 + s1 * s3,
            ...
            -s2,                  s1 * c2,                  c1 * c2;
// todo::
/// R = [n/n.norm() v/v.norm() (nxv)/(nxv).norm()]
    ...
    /// plk坐标是包含尺度信息的，所以法向n不能归一化，方向向量v可以归一化
    /// 因为n/v表示的是原点到直线的距离，如果v归一化了，n就不能归一化，不然点到线的距离就不可观了
    // todo 上面说错了，plk的表示方法是up to scale的
    plk.head(3) = n;
    plk.tail(3) = v;
...
Vector3d plucker_origin(Vector3d n, Vector3d v) {
    /// if v is already normalized, then v.dot(v) is unnecessary
    /// while n can never be normalized

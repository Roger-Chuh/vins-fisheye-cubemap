...
/*cv::Mat DepthEstimator::ComputeDispartiyMap(cv::Mat & left, cv::Mat & right) {
    ...
    if (first_init) {
        ...
        initUndistortRectifyMap(cameraMatrix, cv::Mat(), R2, P2, imgSize, CV_32FC1, _map21,
                                _map22);
//        _map11.upload(_map11);
//        _map12.upload(_map12);
//        _map21.upload(_map21);
//        _map22.upload(_map22);
...
#else
    cv::remap(left, leftRectify, _map11, _map12, cv::INTER_CUBIC);
    cv::remap(right, rightRectify, _map21, _map22, cv::INTER_CUBIC);
#endif

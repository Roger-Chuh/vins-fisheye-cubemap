...
void FisheyeFlattenHandler::imgs_callback(double t, const cv::Mat & img1, const cv::Mat img2, bool is_blank_init) {
    ...
    cv::Mat asfa = cv::imread("/home/roger/vins-fisheye2/src/VINS-Fisheye/data/up_orig_img.png");
    //cv::Mat asfaasd = cv::imread("/home/roger/vins-fisheye2/src/VINS-Fisheye/data/down_orig_img.png");

    //img_up = img1.clone();
    // img_down = img2.clone();
    if (asfa.cols < 10)
    {    imwrite("/home/roger/vins-fisheye2/src/VINS-Fisheye/data/up_orig_img.png", img1);
        imwrite("/home/roger/vins-fisheye2/src/VINS-Fisheye/data/down_orig_img.png", img2);
    }
    ...
    if (USE_GPU) {
        ...
        fisheye_buf_down.push(fisheye_down_imgs_gray);
            ...
            cv::Mat gray_up, gray_down;
            if (img1.channels() > 1) {
                cv::cvtColor(img1, gray_up, cv::COLOR_BGR2GRAY);
                cv::cvtColor(img2, gray_down, cv::COLOR_BGR2GRAY);
            }
            else
            {
                gray_up = img1;
...
//                cv::imshow("up_top_img ", gray1 );
        ...
        fisheye_raw_buf_up.push(gray_up);
        fisheye_raw_buf_down.push(gray_down);
...
double FisheyeFlattenHandler::pop_from_buffer(
    ...
    if(!USE_GPU) {
        ...
        if (fisheye_buf_t.size() > 0) {
            ...
            up_raw = fisheye_raw_buf_up.front();
            down_raw = fisheye_raw_buf_down.front();
            ...
            fisheye_raw_buf_up.pop();
            fisheye_raw_buf_down.pop();
...
//            if (is_odometry_frame) {
  //              need_to_pack_and_send = true;
   ...
   //         estimator.inputFisheyeImage(cur_frame_t, cur_up_gray_cuda, cur_down_gray_cuda);
            ...
            /// 这其实是一个赋值，cur_up_gray是以引用的形式传进函数的
            cur_frame_t = fisheye_handler->pop_from_buffer(
                cur_up_gray,
                cur_down_gray,
                cur_up_color,
            ...
            /// 处理针孔化后的鱼眼图像
            estimator.inputFisheyeImage(cur_frame_t, cur_up_gray, cur_down_gray, cur_up_raw, cur_down_raw);
        }
        double t_0 = t0.toc();
...
void VinsNodeBaseClass::Init(ros::NodeHandle & n)
{
    ...
    remove("/home/roger/vins-fisheye2/src/VINS-Fisheye/data/concat.png");
    remove("/home/roger/vins-fisheye2/src/VINS-Fisheye/data/up_top_img.png");
    remove("/home/roger/vins-fisheye2/src/VINS-Fisheye/data/down_top_img.png");
    remove("/home/roger/vins-fisheye2/src/VINS-Fisheye/data/up_side_img.png");
    remove("/home/roger/vins-fisheye2/src/VINS-Fisheye/data/down_side_img.png");
    remove("/home/roger/vins-fisheye2/src/VINS-Fisheye/data/up_orig_img.png");
    remove("/home/roger/vins-fisheye2/src/VINS-Fisheye/data/down_orig_img.png");
